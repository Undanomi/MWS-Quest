...
while len(files) > 0:
    ...
    if(item[1] == ''):
        merged_data.append(f'{dt.strftime("%H:%M:%S")} <color=blue>{item[0]}</color>は<color=yellow>{item[2]}</yallow>')
    else:
        merged_data.append(f'{dt.strftime("%H:%M:%S")} <color=blue>{item[0]}</color>は<color=red>{item[1]}</red>で<color=yellow>{item[2]}</yallow>')
